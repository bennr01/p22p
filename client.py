...
# before importing twisted, change reactor to selectreactor
...
try:
	...
	try:
		...
		END_SIGNAL = signal.CTRL_C_EVENT
ORG_SIG_HANDLER = signal.getsignal(END_SIGNAL)
...
class P22PClientProtocol(websocket.WebSocketClientProtocol):
	...
	def clientConnectionFailed(self, connector, reason):
		...
		websocket.WebSocketClientProtocol.clientConnectionFailed(
	...
	def onClose(self, wasclean, code, reason):
		...
		if not wasclean:
			raise CommunicationError(
				"WS-Error {n}: {r}!".format(n=code, r=reason)
				)
		else:
			try:
				reactor.stop()
	...
	def __send_close(self, ai):
		...
		self.send_to(
			peer, ID_PC + "CLOSE" + chr(ai[0]) + struct.pack("!H", ai[1])
			)
	...
	def create_group(self, name, pswd, key=None):
		"""
		creates and joins a group.
		returns True if it was created, otherwise False.
		key is optional and is a key for creating a group with a reserved name."""
		if not self.did_handshake:
			return False
	...
	def relay_port(self, pid, remote, local=0):
		"""
		relay port local to port remote on peer pid.
		A socket can connect to the local port and
		the communication will be relayed to remote.
...
Each list element has the structure:
	(type,from,localport,to,peerport,recv,send).
type: a string
from: 'LOCAL' or int
localport: int
to: 'LOCAL' or int
...
"""
	...
	def reserve_group(self, name, sco=True):
		"""reserves a froup and returns the key.
		if sco, only medsages between creator and clients are allowed."""
...
class P22PClientFactory(websocket.WebSocketClientFactory):
	...
	def __init__(self, root, url):
		websocket.WebSocketClientFactory.__init__(
			self, url=url, protocols=PROTOCOLS,
			useragent=USER_AGENT, debug=DEBUG
			)
...
class P22PClient(object):
	...
	def set_whitelist(self, wl):
		...
		if not (isinstance(wl, list) or isinstance(wl, tuple) or (wl is None)):
			raise ValueError("wl needs to be a list, tuple or None!")
...
class P22PClientConsole(cmd.Cmd):
	...
	def do_relay(self, cmd):
		...
		try:
			...
			self.stdout.write(
				"Error: Invalid Argument or invalid number of arguments!\n"
				)
			return
		p = self.client.relay(peer, remote, local)
		self.stdout.write(
			"Now relaying port {l} to {r}.\n".format(l=p, r=remote)
			)
...
"""
		try:
			...
			if ct == "ALL":
				...
				raise Exception("see except-handler")
		except:
			self.stdout.write("Error: Invalid Argument!\n")
			return
	...
	def do_extend(self, cmd):
		...
		if not nk:
			...
			try:
				...
				self.stdout.write(
					"Error: Cannot save new key! Please savt this to {p}:\n{k}\n".format(
						p=kf, k=nk
						)
					)
				return
...
def _signhandler(sign, frame):
	"""handles a signal.signal."""
	_stop()
	sys.exit(0)
...
if __name__ == "__main__":
	...
	if len(sys.argv) == 1 or (len(sys.argv) == 2 and ("-d" in sys.argv)):
		...
		parser = argparse.ArgumentParser(
			description="Connect to a P22P-Network"
			)
		parser.add_argument(
			"-a", action="store", dest="address",
			default=DEFAULT_SERVER, help="address of server"
			)
		parser.add_argument(
			"command", action="store", help="What to do",
			choices=("create", "join")
			)
		parser.add_argument(
			"name", action="store", help="Name of Group"
			)
		parser.add_argument(
			"pswd", action="store", help="Password for Group"
			)
		parser.add_argument(
			"-k", action="store", dest="keyfile",
			help="Keyfile used for creating the Group"
			)
		parser.add_argument(
			"-n", action="store_false", dest="extend",
			help="do not extend keyfile"
			)
		# parser.add_argument("-t",action="store",dest="type",help="Type of Network",choices=("ALL","SCO"))
		parser.add_argument(
			"-p", action="store", dest="conns",
			help="ports to relay as PEER:LOCAL:REMOTE", nargs="+",
			type=_to_portlist, default=[], metavar="CONN"
			)
		parser.add_argument(
			"-w", nargs="+", type=int, dest="whitelist",
			default=None, help="ports to whitelist (default: all)",
