...
DB_PATH = os.path.join(
	os.getenv("OPENSHIFT_DATA_DIR", os.getcwd()),
	"rgk.dbm")
SECRETPATH = os.path.join(
	os.getenv("OPENSHIFT_DATA_DIR", os.getcwd()),
	"secret.dat"
)
DEBUG = ("-d" in sys.argv)
...
class ReservedGroupKey(object):
	"""
	This class represents a sicned key for restricting
...
class P22PServerProtocol(websocket.WebSocketServerProtocol):
	...
	def processHandshake(self):
		"""
		for Openshift proxy compatibility,
		replace 'HEAD'-Method with 'GET'-Method.
		"""
		if ("GET" not in self.data) and ("HEAD" in self.data):
				...
				if payload == "LEAVE":
					...
					if gn in self.factory.groups.keys():
						...
						if state is False:
							...
							self.cid = state
							self.sendMessage(
								ID_CTRL + "I:JOIN" + chr(self.cid), True
								)
					else:
						self.sendMessage(ID_CTRL + "E:NOJOIN", True)
					...
					if not cont:
						...
						if gn in self.factory.groups.keys():
							# only this client has the permission to
							# have such a group, destroy the old one
							self.factory.groups[gn].destroy()
						self.factory.groups[gn] = Group(
							self.factory, gn, pswd, sco
							)
						self.cid = self.factory.groups[gn].join(self, pswd)
						self.sendMessage(
							ID_CTRL + "I:CREATE" + chr(self.cid), True
							)
					elif gn in self.factory.groups.keys() or gn.startswith("#"):
						self.sendMessage(ID_CTRL+"E:NOCREATE", True)
					else:
						self.factory.groups[gn] = Group(
							self.factory, gn, pswd, False
							)
						self.cid = self.factory.groups[gn].join(self, pswd)
						self.sendMessage(
							ID_CTRL + "I:CREATE" + chr(self.cid), True
